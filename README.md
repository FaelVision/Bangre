...
**Préparation à la connexion.** Dès qu'un utilisateur est connecté avec du réseau (et une fois par session ensuite), `src/lib/offline-ready.ts` prépare l'appareil en une fois : le service worker télécharge l'application hors ligne et **tout ce qu'elle charge** — scripts cités dans la page, mais aussi ceux que React charge à la demande (listés dans les données de la page) et, de proche en proche, ceux que ces scripts nomment ; feuilles de style, polices, logo, icônes — pendant que la copie de l'école est téléchargée. La barre latérale affiche le résultat : « ✓ Prêt à fonctionner hors ligne », ou « Mode hors ligne incomplet » avec un bouton « Préparer maintenant ». Les images sont servies telles quelles (`images.unoptimized`) : via `/_next/image`, le logo disparaissait hors ligne.
...
L'export Excel des retards se fait aussi hors ligne, écrit par l'appareil à partir des lignes affichées. L'**import d'une liste d'élèves** (Excel ou CSV) fonctionne hors ligne : le fichier est lu sur l'appareil et chaque élève part dans la file. La **configuration d'une classe** est consultable hors ligne (montants, tranches, échéances, messages de rappel) et se modifie en ligne.

**Ce qui reste en ligne** (le shell l'annonce clairement au lieu d'échouer) : les exports PDF et les reçus PDF, la création d'une classe et la modification de sa configuration, le passage d'année, l'abonnement et l'administration. Il n'y a pas non plus de résolution de conflits multi-appareils : deux postes qui modifient la même fiche hors ligne appliqueront leurs versions dans l'ordre d'arrivée, la dernière l'emportant.
