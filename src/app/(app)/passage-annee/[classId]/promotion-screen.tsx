...
export function PromotionScreen({
  ...
  return (
    <div>
      <div className="h-[70px] border-b border-(--color-border) flex items-center gap-3 px-7">
        <div>
          <div className="text-[12.5px] text-(--color-text-muted)">Passage d&apos;année</div>
          <div className="text-[19px] font-semibold tracking-tight mt-0.5 flex items-center gap-2">
            {className} →{" "}
            <input
              value={targetName}
              onChange={(e) => setTargetName(e.target.value)}
              className="border-b border-dashed border-(--color-border-strong) bg-transparent focus:outline-none focus:border-(--color-primary) w-40"
            />
          </div>
        </div>
        <select
          ...
          className="h-9 border border-(--color-border-strong) rounded-lg bg-white px-2.5 text-[12.5px]"
      ...
      <div className="bg-(--color-gold-bg) border-b border-(--color-gold-border) px-7 py-4 flex items-center gap-3.5">
      ...
      <div className="p-4 px-7 pb-10 grid gap-4" style={{ gridTemplateColumns: "1fr 288px", alignItems: "start" }}>
        <div>
            ...
            {mainStudents.map((s) => {
              ...
              return (
                <div
                  key={s.id}
                  onClick={() => toggle(s.id)}
                  className="flex items-center gap-3.5 px-4 py-3 border-b border-(--color-border-row) last:border-b-0 cursor-pointer hover:bg-(--color-bg-subtle)"
                ...
                >
                  ...
                  <span
                    className="text-[13px] w-[130px] text-right"
                  ...
                  <button
                    ...
                    className="text-[12px] text-(--color-text-muted) hover:text-(--color-danger-text) cursor-pointer ml-1"
                    title="Cas particulier (transfert / sortie)"
                  >
                    ⋯
                  </button>
                </div>
            ...
            <div className="grid gap-2 mt-3">
              {specialIds.map((s) => (
                <div key={s.id} className="flex justify-between items-center text-[13px]">
                  <span>
                    {s.lastName} {s.firstName} — {decisions[s.id] === "transferred" ? "Transféré" : "Sorti"}
                  </span>
                  <button onClick={() => setDecision(s.id, "promote")} className="text-xs text-(--color-primary) font-semibold cursor-pointer">
                    Annuler
                  </button>
              ...
              {specialIds.length === 0 && (
                <div className="text-[12.5px] text-(--color-text-muted)">
                  Cliquez « ⋯ » sur une ligne pour marquer un transfert ou une sortie.
                </div>
              )}
            </div>
          </div>
