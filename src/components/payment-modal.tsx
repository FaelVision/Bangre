...
import { enqueuePayment } from "@/lib/offline-queue";
...
export function PaymentModal({
  ...
  const loadContext = useCallback((id: string) => {
    ...
    startTransition(async () => {
      try {
        ...
        // No network reachable at all (not just a slow/offline flag) — fall
        // back to the reduced offline form instead of spinning forever.
        setOfflineFallback(true);
        setMode("partial");
      }
    });
  ...
  return (
    <div className="fixed inset-0 bg-[#281C14]/42 flex items-start justify-center p-8 z-50 overflow-auto">
      <div className="flex flex-wrap gap-4 items-start max-w-full" style={{ width: 1000 }}>
        <div className="flex-[1.3] min-w-[min(420px,100%)] bg-(--color-bg-app) rounded-[18px] shadow-2xl overflow-hidden">
          <div className="px-5.5 py-4.5 border-b border-(--color-border) flex items-center">
            <div>
          ...
          <div className="p-5.5">
            ...
            {studentId && !offlineFallback && context && context !== "error" && !result && (
              <>
                ...
                <div className="flex items-center gap-4 border border-(--color-border) bg-(--color-bg-subtle) rounded-xl px-4 py-3.5 mt-4">
                  <div>
        ...
        {context && context !== "error" && (
          <div className="w-[300px] flex-1 max-w-full bg-white rounded-[18px] shadow-2xl p-5.5">
