...
/**
 * The dashboard, as pure presentation. The page feeds it from the database and
 * the offline shell feeds it from the device's local copy — same screen, same
 * numbers, whichever side the data came from.
 */
export type DashboardViewData = {
  ...
  soldeCount: number;
  partielCount: number;
  rienPayeCount: number;
  lateCount: number;
  billedStudentCount: number;
  unbilledStudentCount: number;
  classSummaries: { class: { id: string; name: string }; studentCount: number; percent: number | null }[];
  recentPayments: {
    id: string;
    amount: number;
    method: string;
...
export function DashboardView({ data, yearLabel }: { data: DashboardViewData; yearLabel: string }) {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-5">
            ...
            <Link href="/retards" className="no-underline hover:no-underline">
              <div className="border border-(--color-danger-border) bg-(--color-danger-bg-soft) rounded-[11px] p-3.5 h-full">
                ...
                <div className="text-[22px] font-bold mt-1 tabular-nums text-(--color-danger-text)">
                  {data.rienPayeCount}
                </div>
