...
export type ClassOverviewRow = {
  ...
  studentCount: number;
  percent: number | null;
  status: "a_jour" | "partiel" | "retard" | "non_definie";
};

/**
 * The class grid. `offline` hides what needs the server (creating or
 * configuring a class), so the screen never offers an action that cannot work.
 */
export function ClassesView({
  ...
  const chips = [
    ...
    ...LEVELS.map((l) => ({ key: l as string, label: l as string })),
  ...
  return (
    <div>
      ...
      <div className="p-4 lg:p-5 lg:pt-5 pb-10 lg:px-7">
              ...
              return (
                <Card key={c.id} className="border-(--color-gold-border)">
                  ...
                  <div className="text-[12.5px] text-(--color-gold-text) bg-(--color-gold-bg) rounded-lg px-3 py-2.5 mt-3 leading-relaxed">
                    {offline
                      ? "Le montant et les tranches se définissent en ligne."
                      : "Définissez le montant et les tranches pour activer le suivi des paiements."}
                  </div>
                  {offline ? (
                    <Link
                      href={`/classes/${c.id}/eleves`}
                      className="h-9 rounded-[9px] border border-(--color-border-strong) bg-(--color-bg-subtle) flex items-center justify-center text-[13px] font-semibold mt-3.5 no-underline hover:no-underline"
                    >
                      Voir les élèves
                    </Link>
                  ) : (
