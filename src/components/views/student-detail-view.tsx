...
export type StudentDetailData = {
  student: {
    ...
    reminders: { id: string; sentAt: Date }[];
  };
  summary: StudentSummary;
};

/**
 * One student's file. Offline, the receipt PDFs are the only thing missing —
 * they are generated by the server — so those links step aside rather than
 * leading to an error page.
 */
export function StudentDetailView({ data, offline = false }: { data: StudentDetailData; offline?: boolean }) {
  ...
  const studentHint = `${student.lastName} ${student.firstName} · ${student.class.name} · ${student.matricule}`;
          ...
          {summary.status !== "non_defini" ? (
            ...
            <Card className="border-(--color-gold-border) bg-(--color-gold-bg)">
              <div className="text-[13px] text-(--color-gold-text) leading-relaxed">
                ...
                {offline ? (
                  <span className="font-semibold">À configurer en ligne.</span>
                ) : (
                  ...
                  >
                    <div className="flex-1 min-w-[150px]">
                      ...
                      <div
                        ...
                        style={{
                          color: ts.status === "late" ? "var(--color-danger-text)" : "var(--color-text-muted)",
                        }}
                      >
                      ...
                      <PayButton
                        ...
                        className={`h-8 rounded-lg flex items-center px-3.5 text-[12.5px] font-semibold ${
                          ts.status === "late"
                            ...
                            : "border border-(--color-border-strong) bg-white"
                        }`}
                      >
                        Encaisser
                      </PayButton>
